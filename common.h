...
DESCRIPTION:
    ...
    First, print @str and followed by a colon and a blank. Then an error message corresponding to the value of @err and a new-line.

TIME: 2022/9/10
*/
__attribute__ ((always_inline))
__attribute__((nonnull)) static inline void show_err(const char *str,
                                                       const int err)
{
...
NOTE:
    The C standard makes no guarantees as to when output to stdout (standard output) or stderr (standard error) is actually flushed. Before program being terminated, output stream requires to flush and close.
    Typically, the clearout() funtion will call by the the atexit() function in order to close @stdout and @stderr at the end of the program. 
    Reference: https://wiki.sei.cmu.edu/confluence/display/c/FIO23-C.+Do+not+exit+with+unflushed+data+in+stdout+or+stderr
...
RETURN VALUE:
    ...
    On error, return error value and set @errno. The show_err() function and the show_errno() function translate @errno to a human readable string.
...
/*
DESCRIPTION:
    Test return value of the getnum() function. And print error message to @stderr.
...
*/
__attribute__ ((always_inline))
__attribute__((nonnull)) static inline int check_getnum(int err,
                                                        const char *str)
{
    switch (err) {
    case NODIG_ERR:
